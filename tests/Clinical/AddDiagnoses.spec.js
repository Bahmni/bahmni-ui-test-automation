...
//Add certainty to the diagnosis
  ...
  const certaintyDropdowns = await page.getByRole('combobox', { name: 'Diagnoses Certainty' }).all();
...
//Add certainty to the diagnosis
  ...
  const certaintyCount = await page.locator('//div[@data-testid="selected-item"]').count();
  ...
  for (let i = 0; i < certaintyCount; i++) {
    await page.locator(`//div[starts-with(@id,'diagnoses-certainty-dropdown-')][${i}]`).getByText('Confirmed').click();
  }
...
//count the number of li elements under the ul with id downshift-«r5n»-menu
  const liCount = await page.locator('//ul[@id="downshift-«r5n»-menu"]/li').count();
